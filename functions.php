...
if ( ! function_exists( 'penci_load_scripts' ) ) {
	...
	function penci_load_scripts() {
		...
		// Enqueue style
		wp_enqueue_style( 'penci_style', get_stylesheet_directory_uri() . '/style.css', array(), '2.6.1' );

		// Enqueue script
		wp_enqueue_script( 'libs-js', get_template_directory_uri() . '/js/libs-script.min.js', array( 'jquery' ), '2.6.1', true );
		...
		wp_enqueue_script( 'main-scripts', get_template_directory_uri() . '/js/main.js', array( 'jquery' ), '2.6.1', true );
		wp_enqueue_script( 'penci_ajax_like_post', get_template_directory_uri() . '/js/post-like.js', array( 'jquery' ), '2.6.1', true );
		wp_localize_script( 'penci_ajax_like_post', 'ajax_var', array(
				'url'   => admin_url( 'admin-ajax.php' ),
				'nonce' => wp_create_nonce( 'ajax-nonce' )
			)
		);
		if( get_theme_mod( 'penci_page_navigation_ajax' ) ) {
			wp_enqueue_script( 'penci-images-loaded', get_template_directory_uri() . '/js/imagesloaded.pkgd.js', array( 'jquery' ), '1.0', true );
			wp_enqueue_script( 'penci_ajax_more_posts', get_template_directory_uri() . '/js/more-post.js', array( 'jquery' ), '1.0', true );
...
if ( ! function_exists( 'penci_custom_markup_for_gallery' ) ) {
	...
	function penci_custom_markup_for_gallery( $string, $attr ) {
		...
		if( $type == 'single-slider' ):
			$output .= '<div class="penci-slick-slider" data-auto-height="true">';
		endif;
